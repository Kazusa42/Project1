...
class Attention(nn.Module):
    def __init__(self, *, dim, fmap_size, heads=4,
                 dim_head=128, rel_pos_emb=False):
        super().__init__()
        self.heads = heads
...
class BottleBlock(nn.Module):
    def __init__(self, *, dim, fmap_size, dim_out, proj_factor=4, downsample=True,
                 heads=4, dim_head=128, rel_pos_emb=False, activation=nn.GELU()):
        super().__init__()
        ...
        self.net = nn.Sequential(
            ...
            activation,
            Attention(dim=attn_dim_in, fmap_size=fmap_size, heads=heads,
                      dim_head=dim_head, rel_pos_emb=rel_pos_emb),
...
class BottleStack(nn.Module):
    def __init__(self, *, dim, fmap_size, dim_out=2048, proj_factor=4,
                 num_layers=3, heads=4, dim_head=128, downsample=True,
                 rel_pos_emb=False, activation=nn.GELU()):
        super().__init__()
        ...
        for i in range(num_layers):
            ...
            layers.append(BottleBlock(dim=dim, fmap_size=layer_fmap_size, dim_out=dim_out,
                                      proj_factor=proj_factor, heads=heads, dim_head=dim_head,
                                      downsample=layer_downsample,
                                      rel_pos_emb=rel_pos_emb, activation=activation))
