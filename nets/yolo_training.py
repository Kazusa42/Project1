...
class YOLOLoss(nn.Module):
    ...
    def forward(self, layer_num, input_batch, targets=None):
        ...
        y_true, noobj_mask, box_loss_scale = self.get_target(layer_num, targets, scaled_anchors, in_h, in_w)
        noobj_mask, pred_boxes = self.get_ignore(layer_num, x, y, h, w, targets, scaled_anchors, in_h, in_w, noobj_mask)
    ...
    def get_target(self, layer, targets, anchors, in_h, in_w):
        bs = len(targets)
        noobj_mask = torch.ones(bs, len(self.anchors_mask[layer]), in_h, in_w, requires_grad=False)
        box_loss_scale = torch.zeros(bs, len(self.anchors_mask[layer]), in_h, in_w, requires_grad=False)
        y_true = torch.zeros(bs, len(self.anchors_mask[layer]), in_h, in_w, self.bbox_attrs, requires_grad=False)
        ...
        for b in range(bs):
            ...
            for t, best_n in enumerate(best_ns):
                if best_n not in self.anchors_mask[layer]:
                    continue
                k = self.anchors_mask[layer].index(best_n)
